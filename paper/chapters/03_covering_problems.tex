...
\end{align*}
The pair of $(X, S)$ is the input for Exact Cover. After definiting the reduction function,
...
In all cases, we have to iterate the set of clauses, even though it is not necessary in the mathematical reduction, 
...
Nevertheless, the proof was not automated and required much efforts, 
...
an injective function between the preimage and the image. Then we can obtain an inequality relationship.
\begin{figure}
    \centering
...
is also included in the $c_u$, for $s \in S'$ and $S' \subseteq S$. Thus, it is the only element in the set 
$W \cap c_u$. Consequently, $W$ hits $S$ exactly.
\end{proof}
\begin{lemma}[Completeness]
...
Finally, we show that the reduction is polynomial. 
\begin{lemma}[Polynomial Complexity]
