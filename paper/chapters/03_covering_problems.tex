...
Set covering problems ask whether a certain combinatorial structure $A$ covers another structure $B$. 
...
\label{eq:0} }
\begin{myalign}
    \textbf{Exact Cover}_{YES} &:= \{(X, S) | \bigcup S \subseteq X \land \exists S' \subseteq S.\\
    & \forall x \in X. \exists s \in S'. x \in s\\
    & \land\forall s\ t \in S'. s \neq t \longrightarrow s \cap t = \emptyset
    \}
\end{myalign}
We call $(X, S)$ a YES-instance of Exact Cover and $S'$ an exact cover of $X$. In other words, $S'$ covers $X$ 
exactly.
...
\end{align*}
\begin{definition}[Reduction Satisfiability to Exact Cover]
...
one index, whereas a binary set of a clause and a position needs at least two indices. 
...
As a result, we choose the maximum of all metrics and use it as our sole metric\footnote{A few previous reductions in Karp21 project use the number of clauses 
...
The NREST-algorithm collects the sets of the variables, clauses, and literals first. 
...
\begin{definition}[Reduction Exact Cover to Exact Hitting Set]
...
In the same way as the implementation for the reduction from Satisfiability to Exact Cover, we can only use one metric. 
...
The proof-automation will then fail in showing $|C| \leq |S| \cdot |S| + 1$ when $|S| \geq |X|$. 
