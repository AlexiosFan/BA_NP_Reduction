...
\section{Subset sum}
We define the Subset Sum problem with a set and a weighting function. There is also an alternative definition using a multiset or a list without 
a weighting function, which is useful for our other reductions. More details follow in the Partition section. 
...
We reduce Exact Cover to the Subset Sum. We start with Exact Cover over natural numbers. 
...
The triple $(S, w, B)$ is then an input for Subset Sum. With the definition, we show the correctness of the reduction.
...
\end{align*}
Thus, $(S, w, B)$ is an instance of the Subset Sum. 
\end{proof}
...
over natural numbers. Then, we perform a complexity analysis over the whole construction.
\begin{lemma}[Polynomial Complexity]
    The construction of $(S, w, B)$ from $(X, S)$ can be computed within polynomial time. 
\end{lemma}
\begin{proof}
...
\end{proof}
Then we summarize to obtain the main theorem.
\begin{theorem}
...
a mapping with the property in Lemma 9, in whose definition the predicate \textbf{SOME} is used again.
...
Besides the proof presented in the implementation details, it is necessary to show that the representation of a natural number as
 polynomials with the base $p$ is unique. To achieve this, we imported the Archive of Formal Proofs entry DigitInBase and applied the theorem 
...
The implementation of the proof for polynomial complexity is identical to what is introduced in reductio details. 
...
\subsection{Example}
Again, we present an example for the reduction from Exact Cover to Subset Sum for better understanding and 
readability.\\\\
...
\section{Subset Sum in Sequence and Partition}
