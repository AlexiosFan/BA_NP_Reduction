...
The we construct a set $X$ and its collection $S$
  - $X = \{x_n | x_n \in (\text{vars } F)\} \cup \{c_i | c_i \in F\} \cup \{p_{ij} | p_{ij} \in c_i \land c_i \in F\}$
    </br>X includes all variables, all clauses and all atoms in $F$
  - $(x_n, \bot) = \{x_n\} \cup \{p_{ij} | p_{ij} = \neg x_n \land p_{ij} \in c_i \land c_i \in F\}$
  - $(x_n, \top) = \{x_n\} \cup \{p_{ij} | p_{ij} = x_n \land p_{ij} \in c_i \land c_i \in F\}$
  </br> bipartites the existence of $x_n$ by its positive or negative existence
  - $S = \{\{p_{ij}\} | p_{ij} \in c_i \land c_i \in F\} \cup \{(x_n, \bot) | x_n \in (\text{vars } F)\} 
         \cup \{(x_n, \bot) | x_n \in (\text{vars } F)\}  \cup \{\{c_i, p_{ij}\} | c_i \in F \land p_{ij} \in c_i\}$
...
Now we show that the construction is sound and complete:
  - Soundness: $M \models F \Longrightarrow S' \text{ covers } X$
    ...
    2. to include all $C_i$, we include $\{C_i, p_{ij}\}$ for $M(p_{ij}) \equiv \top$,
    ...
    3. in case there are more than on such $p_{ij}$, 
       we choose the smallest index $j$ and include all other atoms as elementary set \{p_{ij}\} 
    </br>
    Then we check for the correctness:
    1. each $x_n$ is covered in its $(x_n, \neg M(x_n))$
    2. each $C_i$ is covered in its $\{C_i, p_{ij}\}$
    3. each $p_{ij}$ is covered either in $(x_n, \neg M(x_n))$ if $M(p_{ij}) \equiv \bot$
      </br> or in $\{C_i, p_{ij}\}$ and $p_{ij}$ if $M(p_{ij}) \equiv \top$
  ...
  - Completeness: $S' \text{ covers } X \Longrightarrow M \models F$
    ...
    </br> This is trivial, for  each $p_{ij}$ in $\{C_i, p_{ij}\}$ is not included in any $(x_n, b_n) \in S'$. The reason is 
